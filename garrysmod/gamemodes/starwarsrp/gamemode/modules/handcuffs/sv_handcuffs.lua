...
hook.Add( "SetupMove", "Handcuffs_SetupMove", function(ply, mv, cmd)
	...
	-- if kidnapper:IsPlayer() and ply:GetPos().z+30 < kidnapper:GetPos().z and ply:OnGround() then
	-- 	-- ply:Jump()
	-- end
	-- print(dir[3])
	-- dir[3] = dir[3]
...
function pMeta:SetHandcuffed(bool,player)
    if bool then
...
function pMeta:ProgressCuffed(bool,player)
...
netstream.Hook('ProgressCuffed_Succes',function(player,data)
	local bool = player.VarProgressCuffedBool
	local duration = bool and HANDCUFFED_DURATION or UN_HANDCUFFED_DURATION

	if bool and player.VarProgressCuffed.GetPlayerHandcuffed then return end
