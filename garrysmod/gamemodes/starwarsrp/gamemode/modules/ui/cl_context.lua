...
local function OpenNewTask(pl, i )
	...
	for k, v in pairs(FEATURES_TO_NORMAL) do
		...
		features_buttons[k]:SetPos( 0, (20*(fi-1)) )
	...
	Rank.Paint = function( self, w, h )
		draw.RoundedBox(2,0,0,w,h,Color(255, 255, 255, 5))
	...
	Rank.DoClick = function( self )
		...
		for i = 1, self.Menu:ChildCount() do
			...
			pnl.Paint = function( self, w, h )
				draw.RoundedBox(2, 0, 0, w, h,Color(73, 111, 158, 255))
			end
	...
	Close.Paint = function( self, w, h )
		draw.RoundedBox(2, 0, 0, w, h, Color(191, 67, 57))
	...
	teams_search.Paint = function( self, w, h )
		draw.RoundedBox(2,0,0,w,h,Color(255, 255, 255, 35))
		self:DrawTextEntryText(Color(255,255,255,240), Color(0,165,255,255), Color(255,255,255,240))
	end
	...
	DListTeams.VBar.Paint = function( self, w, h ) end
	DListTeams.VBar.btnGrip.Paint = function( self, w, h ) draw.RoundedBox(2, 0, 0, w, h, Color(255, 255, 255, 190)) end
	...
	for _, colum in pairs(DListTeams.Columns) do
		...
		colum.Header.Paint = function( self, w, h )
			draw.RoundedBox(2, 0, 0, w, h, Color(73+5, 111+5, 158+5))
	...
	Save.Paint = function( self, w, h )
		draw.RoundedBox(2, 0, 0, w, h, Color(119,184,0))
	...
	Save.Paint = function( self, w, h )
		draw.RoundedBox(2, 0, 0, w, h, Color(41,163,246))
	...
	Time.Paint = function( self, w, h )
		draw.RoundedBox(2, 0, 0, w, h, Color(189,155,234))
...
local function OpenContextMenu()
	...
	if IsValid(Menu) then
		...
		RadioWang:SetSize(130, 30)
		RadioWang:SetPos( Menu:GetWide()-306,10)
		RadioWang:SetPaintBorderEnabled( true )
		RadioWang:SetFont('font_base_small')
		-- RadioWang:SetText('')
		...
		RadioWang.Paint = function( self, w, h )
			draw.RoundedBox(2,0,0,w,h,Color(52, 73, 94, 190))
			self:DrawTextEntryText(Color(255,255,255,240), Color(0,165,255,255), Color(255,255,255,240))
		end
		...
		Person.Paint = function( self, w, h )
			...
			draw.SimpleText('Третье лицо', "font_base_small", w/2, h/2, Color( 255, 255, 255, 255 ), 1, 1)
		end
		Person:SetZPos( -10 )
		...
		if LocalPlayer():IsAdmin() then
			local Admin = vgui.Create( "DButton", Menu )
			Admin:SetSize( 162, 30 )
			Admin:SetText('')
			Admin:SetPos( Menu:GetWide()-10-162, 236 )
			Admin:SetZPos(10)
			Admin.Paint = function( self, w, h )
				draw.RoundedBox(2, 0, 0, w, h, Color(52, 73, 94))
				draw.SimpleText('Админ мод', "font_base_small", w/2, h/2, Color( 255, 255, 255, 255 ), 1, 1)
			end
			-- Admin:SetZPos( -10 )

			Admin.DoClick = function( self )
				show_adminmode = not show_adminmode
				-- Menu:Remove()
			end
		end
		...
		if IsValid(LocalPlayer():GetNW2Entity( 'sneakyjetpack' )) then
			...
			MuguPidoras:SetSize( 162, 31 )
			MuguPidoras:SetText('')
			MuguPidoras:SetPos( Menu:GetWide()-504, 10 )
			MuguPidoras:SetZPos(10)
			MuguPidoras.Paint = function( self, w, h )
				draw.RoundedBox(2, 0, 0, w, h, Color(231,108,54))
				draw.SimpleText('Снять джетпак', "font_base_small", w/2, h/2, Color( 255, 255, 255, 255 ), 1, 1)
			end
			MuguPidoras:SetZPos( -10 )
		...
		--     draw.RoundedBox(2,0,0,w,h,Color(64, 105, 153, 190))
		--     self:DrawTextEntryText(Color(255,255,255,240), Color(0,165,255,255), Color(255,255,255,240))
		-- end
		...
		ZSlider:SetSize( 162, 20 )
		ZSlider:SetPos( Menu:GetWide() - ZSlider:GetWide() - 10, 212 )
		...
		ZSlider.PerformLayout = function()
			...
			ZSlider.Slider.Knob.Paint = function( self, w, h )
				...
				draw.RoundedBox(2,pos_x,0,w,h,col)
			end
		end
		...
		--             draw.RoundedBox(2,0,0,w,h,Color(52, 73, 94, 195))
		...
		do
			...
			for command, act in pairs(tbl) do
				...
				btn:SetPos(Menu:GetWide() - btn:GetWide() - 10, 250+(i*24))
				btn:SetText('')
				btn.Paint = function( self, w, h )
					draw.RoundedBox(2,0,0,w,h,Color(52, 73, 94, 240))
					draw.SimpleText(act.text, "font_base_small", w*.5, h*.5, Color( 255, 255, 255, 255 ), 1, 1)
	...
	RadioScroll:SetSize( 162, 162 )
	RadioScroll:SetPos( Menu:GetWide() - 162 - RadioScroll:GetWide() - 14, 50 )
	RadioScroll.Paint = function( self, w, h )
		-- draw.RoundedBox(2,0,0,w,h,ColorAlpha(team.GetColor(v:Team()), 90))
	end
		...
		for i, pl in pairs(players) do
			...
			PlayerPanels[i].Paint = function( self, w, h )
				...
				if pl and pcol then
					...
					draw.RoundedBox(2, w-wt, 0, wt, h, Color(pcol.r-12,pcol.g-12,pcol.b-12,255))
					...
					if self:IsHovered() then
						draw.RoundedBox(2,0,0,w,h,Color(230, 230, 230, 4))
					end
				end
			end
		...
		do
			if TEAMS_CANUSE_DEFCONS[LocalPlayer():Team()] then
				...
				for name, text in pairs(DEFCON_TYPES) do
					...
					defcons_buttons[i]:SetSize(160,22)
					defcons_buttons[i]:SetPos(Menu:GetWide() - defcons_buttons[i]:GetWide() - 10 - 165, 250+(24*i))
					defcons_buttons[i]:SetText('')
					defcons_buttons[i].Paint = function( self, w, h )
						draw.RoundedBox(2,0,0,w,h,Color(52, 73, 94, 195))
						draw.SimpleText(name, "font_base_small", w/2, h/2, Color( 255, 255, 255, 255 ), 1, 1)
