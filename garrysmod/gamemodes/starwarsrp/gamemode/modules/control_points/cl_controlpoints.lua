...
hook.Add( "HUDPaint", "ControlPoints_HUDPaint", function()
	...
	local col = ColorAlpha(color_white,cin * 100+40)
	...
	for k, ent in pairs(controls) do
	    ...
	    render.SetStencilEnable(true)
	        ...
	        draw.Arc( {y =scr.y, x =scr.x}, 0, 360, 40*scale, 6, 10*scale, Color(255,255,255,255) )
		...
		draw.Arc( {y =scr.y, x =scr.x}, 0, 360, 27*scale, 6, 27*scale, ColorAlpha(fraction.color,140) )

        if occupied ~= 1 then
		    draw.Arc( {y =scr.y, x =scr.x}, 0, 360, 40*scale+point_active, 6, 1, Color(255,255,255,255-point_active*28) )
        end

		local dist = math.sqrt(LocalPlayer():GetPos():DistToSqr(ent:GetPos()))*0.01905
		local icon_size = (32*scale)

		draw.Icon( scr.x-icon_size/2, scr.y-icon_size/2, icon_size, icon_size, CONTROLPOINT_ICONS[ent:GetNWString('Icon')] or mat_cpp, color_white )
