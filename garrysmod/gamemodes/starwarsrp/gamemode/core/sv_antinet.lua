...
MsgC(Color( 255, 255, 255 ),snteloaded.."\n")

--[[

Original idea     > meep  (https://steamcommunity.com/profiles/76561198050165746)
Code          
                > Maks  (https://steamcommunity.com/profiles/76561198197775845)
                > Zaros (https://steamcommunity.com/profiles/76561198258872399)

Net searcher - Update 26
                > Yoh   (https://steamcommunity.com/profiles/76561198053559858)
                > Zaros (https://steamcommunity.com/profiles/76561198258872399)
                
--]]

local rdmNetNum = math.random(15, 23) -- having more exploits than that means it's DarkRP lol
...
for k in pairs(allBanMethods) do
	helpMsg = "-" .. k .. "\n"
end
...
cvars.AddChangeCallback("snte_banmethod", function(_, oldValue, newValue)
	if !allBanMethods[newValue] then
	...
	if !allBanMethods[newValue].check() then
...
local function instantBan(ply, netCalled)
    ...
    if allBanMethods[banMethod].check() then
        ...
        print("(SNTE) ERROR: ADDON " .. banMethod .. " DOESN'T SEEM TO WORK / BE INSTALLED. FALLBACK TO BASE.")
...
timer.Simple(1, function()
	...
	for i = bad_num, 1, -1 do
		if util.NetworkStringToID(bad_nets[i]) ~= 0 then
			...
			print([[(SNTE) backdoor net "]] .. backdoorNet .. [[" has been detected ! Check your addons and make sure to remove the backdoor]])

			if isfunction(net.Receivers[backdoorNet]) then
				local backdoorInfos = debug.getinfo(net.Receivers[backdoorNet], "S")
				print([[(SNTE) MORE INFO: "]] .. backdoorNet .. [[" was declared in ]] .. backdoorInfos.short_src .. [[ line ]] .. backdoorInfos.linedefined)
			end
...
-- block ulx lua_run module, widely used on unknowing superadmins to backdoor server
if file.Exists("ulx/modules/sh/rcon.lua", "LUA") then
	...
	timer.Simple(1, function()
		if !istable(ulx) || !isfunction(ulx.luaRun) then
			return
		end
		...
		cvars.AddChangeCallback("snte_ulxluarun", function(_, oldValue, newValue)
...
--[[
 old basic protection against the ArmDupe crash method
...
hook.Add("CanTool", "SNTEKILLBOUNCYBALLEXPLOIT", function(ply, tr, tool)
