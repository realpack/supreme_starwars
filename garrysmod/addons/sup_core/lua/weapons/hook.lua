...
SWEP.ViewModel = "models/galactic/weapons/vmodels/supgrapple.mdl"
SWEP.UseHands = true
SWEP.CanDrop = true
...
function SWEP:DrawHUD()
	if IsValid( self:GetHook() ) and self:GetHook():GetHasHit() then
		ShadowText( "Длина: "..tostring(self:GetHook():GetDist()), "hatshook_small", ScrW()/2, ScrH()/2+40 )
		ShadowText( (input.LookupBinding("+attack") or "[ПЕРВИЧНЫЙ]"):upper() .. " - Сократить", "hatshook_small", ScrW()/2, ScrH()/2+70 )
		ShadowText( (input.LookupBinding("+attack2") or "[ВТОРИЧНЫЙ ОГОНЬ]"):upper() .. " - Растянуть", "hatshook_small", ScrW()/2, ScrH()/2+85 )
		ShadowText( (input.LookupBinding("+reload") or "[ПЕРЕЗАРЯДКА]"):upper() .. " - Порвать", "hatshook_small", ScrW()/2, ScrH()/2+100 )
		...
		if IsValid( self:GetHook():GetTargetEnt() ) and self:GetHook():GetTargetEnt():IsPlayer() then
			...
			ShadowText( (input.LookupBinding("+use") or "[ДЕЙСТВИЕ]"):upper() .. " - Спрыгнуть", "hatshook_small", ScrW()/2, ScrH()/2+115 )
		end
	...
	if self:Clip1()>=0 then
		ShadowText( "Остатки верёвки: " .. tostring(self:Clip1()), "hatshook_large", ScrW()/2, ScrH()-50 )
	end
