...
SWEP.Category 				= "MetaHub | Взвод Обеспечения"
...
function SWEP:PrimaryAttack()
	...
	local need = (IsValid(tr.Entity) and tr.Entity:IsPlayer()) and math.min(300-tr.Entity:Armor(),self.ArmorAmount) or self.ArmorAmount
	if self:Clip1() >= need and tr.Hit and IsValid(tr.Entity) and tr.Entity:IsPlayer() and tr.Entity:Armor() < 300 then
		...
		if SERVER then
			...
			tr.Entity:SetArmor(math.min(300,tr.Entity:Armor()+need))
...
function SWEP:SecondaryAttack()
	...
	local need = math.min(300-self.Owner:Armor(),self.ArmorAmount)
	if self.Owner:Armor() < 300 and self:Clip1() >= need then
		self:SendWeaponAnim(ACT_VM_PRIMARYATTACK)
		self.Owner:SetAnimation(PLAYER_ATTACK1) --DoAttackEvent()
		self.IdleAnimation = CurTime() + self:SequenceDuration()
		...
		if SERVER then
			...
			self:TakePrimaryAmmo(need)
			self.Owner:SetAnimation(PLAYER_ATTACK1)
			self.Owner:SetArmor(math.min(300,self.Owner:Armor()+need))
