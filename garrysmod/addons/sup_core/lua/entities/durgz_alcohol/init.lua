...
ENT.MODEL = "models/myproject/mesh_0706.mdl"
...
local function shouldnt_do_that_shit(pl)
    return pl == NULL or not pl or pl == nil or not pl:GetActiveWeapon() or pl:GetNWFloat("durgz_alcohol_high_end") < CurTime()
end

function ENT:High(activator,caller)
	self:Say(activator,"ооо, понесло то как, ща кодить третий сервер начну!")
	...
	for i = 1,thing do
		timer.Simple(math.Rand(5,10), function()
			if activator and activator:GetNWFloat("durgz_alcohol_high_end") - TRANSITION_TIME > CurTime() then
