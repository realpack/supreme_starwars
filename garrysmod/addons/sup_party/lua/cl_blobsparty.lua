...
surface.CreateFont( "blobsPartyFontSmall2", {
	font = "Century Gothic",
	size = 20,
	weight = 0,
	extended = true,
...
MyBlobsParty.members = MyBlobsParty.members or {}
local PartyUIMinStatus = false
...
net.Receive("blobsParty:OpenPartyMenu", function()
	...
	blobsPartyMenu.Paint = function(self,w,h)
		...
		draw.SimpleTextOutlined(BlobsPartyConfig.PartyMenu.." - "..blobsPartyMenu.CurrentCategory, "blobsPartyFontSmall2", 60, 25, Color(255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, Color(0,0,0,0))
	...
	blobsPartyMenuClose.Paint = function(self,w,h)
		...
		draw.SimpleTextOutlined("X", "blobsPartyFont", w/2, h/2, blobsPartyCloseButtonColour, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color(0,0,0,0))
	end
	...
	blobsPartyListPartiesButton.DoClick = function()
		...
		BPNameColumn.Paint = function(self,w,h)
			surface.SetDrawColor(BlobsPartyConfig.SecondColor)
			surface.DrawRect(0,0,w,h)
...
net.Receive("blobsParty:PartySettings", function()
	...
	if PlyStatus == "owner" then
		...
		EditPartyFormContainer.Paint = function(self,w,h)
			...
			draw.SimpleText(BlobsPartyConfig.EnGlow, "blobsPartyFont", 5, 200, Color(255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		...
		EnableGlowCheckbox:SetPos(tx+10, 188)
		...
		GlowColorSelector:SetPos(5, 217)
		...
		SetGlowColorButton:SetPos(5, EditPartyFormContainer:GetTall()-40)
...
net.Receive("blobsParty:GetDetails", function()
	...
	blobsPartyDetails.Paint = function(self,w,h)
		surface.SetDrawColor(BlobsPartyConfig.FirstColor)
		surface.DrawRect(0,0,w,h)
		draw.SimpleTextOutlined(PartyDetails.name, "blobsPartyFontSmall2", 60, 25, Color(255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, Color(0,0,0,0))
...
hook.Add("PreDrawHalos", "blobsParty:DrawHalo", function()
	if MyBlobsParty.members and MyBlobsParty.useglow then
		halo.Add(MyBlobsParty.members, MyBlobsParty.glowcolor, 4,4,4, true, true)
	end
end)
...
local function blobsPartyHUD()
	...
	blobsPartyGUI.Container = vgui.Create("DPanel", blobsPartyGUI)
	blobsPartyGUI.Container:SetSize(blobsPartyGUI:GetWide(), blobsPartyGUI:GetTall())
	blobsPartyGUI.Container.Paint = function() end
	blobsPartyGUI.Container:SetVisible(!PartyUIMinStatus)

	blobsPartyGUI.MinMaxButton = vgui.Create("DButton", blobsPartyGUI)
	blobsPartyGUI.MinMaxButton:SetSize(20,20)
	blobsPartyGUI.MinMaxButton:SetPos(230, 0)
	blobsPartyGUI.MinMaxButton:SetText("")
	blobsPartyGUI.MinMaxButton.Paint = function(self,w,h)
		surface.SetDrawColor(BlobsPartyConfig.FirstColor)
		surface.DrawRect(0,0,w,h)
		...
		draw.SimpleText(PartyUIMinStatus and BlobsPartyConfig.DownArrow or BlobsPartyConfig.UpArrow, "DermaDefault", w/2, h/2, Color(255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end

	blobsPartyOwner = vgui.Create("DPanel", blobsPartyGUI.Container)
	...
	PMDetails.Paint = function(self,w,h)
		if IsValid(MyBlobsParty.owner) then
			...
			-- surface.DrawRect(0, h-4, 50 * (blobsPartyOwner.HPLerp/100), 4)
		end
	end

	local blobsPMembers = {}

	blobsPartyGUI.MinMaxButton.DoClick = function()
		blobsPartyGUI.Container:SetVisible(PartyUIMinStatus)
		PartyUIMinStatus = !PartyUIMinStatus

		--[[PMAvatar:SetVisible(PartyUIMinStatus)
		PartyUIMinStatus = !PartyUIMinStatus

		for k,v in pairs(blobsPMembers) do
			if IsValid(v) then
				v:SetVisible(!PartyUIMinStatus)
				v.PMAvatar:SetVisible(!PartyUIMinStatus)
			end
		end

		if (PartyUIMinStatus) then
			blobsPartyHUD()
		end]]--
	end
	...
	for k,v in ipairs(MyBlobsParty.members) do
		if v != MyBlobsParty.owner then
			...
			blobsPMembers[k] = vgui.Create("DPanel", blobsPartyGUI.Container)
			...
			blobsPMembers[k].PMAvatar = vgui.Create("AvatarImage", blobsPMembers[k])
			blobsPMembers[k].PMAvatar:SetSize(50,50)
			blobsPMembers[k].PMAvatar:SetPlayer(v, 50)
