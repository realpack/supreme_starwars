...
function command:OnPlayerExecute(_, target, arguments)
	...
	if (!silent) then
				serverguard.Notify(nil, SERVERGUARD.NOTIFY.GREEN, serverguard.player:GetName(player), SERVERGUARD.NOTIFY.WHITE, " изменил модель ", SERVERGUARD.NOTIFY.GREEN, target:Name(), SERVERGUARD.NOTIFY.WHITE, " на ", SERVERGUARD.NOTIFY.RED, arguments[2]); 
	end
	return true;
end;
...
command.help				= "Выдать профессию";
command.command 			= "setteam";
command.arguments			= {"player", "team"};
command.permissions			= "Set Team";
command.immunity 			= SERVERGUARD.IMMUNITY.LESSOREQUAL;

function command:OnPlayerExecute(_, target, arguments)
	 for k,v in pairs(team.GetAllTeams()) do
        if k == tonumber(arguments[2]) or string.lower(v.Name) == string.lower(arguments[2] or "") then
             target:SetTeam(k, true)
             target:Spawn()
		end
	end
	return true;
...
function command:OnNotify(pPlayer, targets)
	return SGPF("command_setteam", serverguard.player:GetName(pPlayer), util.GetNotifyListForTargets(targets));
end;
...
if (CLIENT) then
	serverguard.netstream.Hook("sgTalkToAdmins", function(data)
		...
		if (util.IsConsole(pPlayer)) then
			chat.AddText(Color(255, 0, 0), "Админ Чат | ", color_white, "Console: " .. text);
			return;
		end;

		if (pPlayer:Name() != pPlayer:Nick()) then
			chat.AddText(Color(255, 0, 0), "Админ Чат | ", team.GetColor(pPlayer:Team()), "(" .. pPlayer:Name() .. ") " .. pPlayer:Nick(), color_white, ": " .. text);
			serverguard.PrintConsole("Админ Чат | (" .. pPlayer:Name() .. ") " .. pPlayer:Nick() .. ": " .. text);
		else
			chat.AddText(Color(255, 0, 0), "Админ Чат | ", team.GetColor(pPlayer:Team()), pPlayer:Nick(), color_white, ": " .. text);
			serverguard.PrintConsole("Админ Чат | " .. pPlayer:Nick() .. ": " .. text);
		end;
	end);
end;
...
if (SERVER) then
    ...
    util.AddNetworkString "sgGiveFriendStatus"
    
    net.Receive("sgGiveFriendStatus", function(_, ply)
        local e = net.ReadEntity()
        if (e:IsPlayer()) then
			if !(ply.sg_Friends) then PlyInit_FetchHTTPFriends(ply) end
            ply.sg_Friends[e] = net.ReadString()
        end
    end)
