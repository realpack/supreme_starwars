...
hook.Add('EntityFireBullets', 'EntityFireBullets_Shields', function(ent, bulletData)
	...
	if hook.GetTable()['EntityFireBullets']['btShield_hook'](ent, bulletData, attacker) then
		return false
	end
	...
	if not tr.Entity:IsWorld() then
		local o_bulletData = table.Copy(bulletData)

		if SERVER then
			-- print(tr.Entity:GetParent())
			-- local parent = tr.Entity:GetParent()

			-- if parent and IsValid(parent) then


			-- 	local damage = (parent:GetCircleHealth() or 100) - 1
			-- 	parent:SetCircleHealth(damage)
			...
			-- EmitSound( "squadshield/impact"..math.random(1,4)..".wav", tr.HitPos, 1, CHAN_AUTO, 30, 100, 0, 100 )

			-- print(parent)
			-- parent:EmitSound( "squadshield/impact"..math.random(1,4)..".wav", 75, 100, 1, CHAN_AUTO )
		-- else
		...
		-- 		-- -- effectdata:SetAngles( Angle(90,90,90) )
		...
		local dist = tr.HitPos:Distance(bulletData.Src)
		bulletData.Distance = 0
		bulletData.Damage = 0
		bulletData.Callback = nil
