		...
		for i = 1, #players do
			...
			if (!IsValid(pPlayer.screenPanel)) then
				...
				function lastBase.capture:DoClick()
					...
					g_ScreenCapData[panel.player:UniqueID()] = {data = {}, parts = 1};
				end;
				...
				function lastBase.view:DoClick()										
					...
					local path = ("serverguard/screencaps/%s.jpg"):format(tostring(panel.player:SteamID64()));
				...
				function lastBase:Paint(w, h)
					if (IsValid(panel.player)) then
						local data = g_ScreenCapData[panel.player:UniqueID()];
