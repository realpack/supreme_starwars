	...
	for i, data in ipairs(restrictionTypes) do
		...
		if (GAMEMODE.IsSandboxDerived and toolsPanel and data.type == "Tools") then
			...
			for k, v in pairs(toolsTable) do
				...
				if (restrictionData[data.name] and isbool(restrictionData[data.name][v.Command])) then
...
plugin:Hook("serverguard.panel.RankEditorCreationMenu", "restrictions.RankEditorCreationMenu", function(list)
	...
	PopulateRestrictionList(restrictionList, toolsList, newRankRestrictions, {});
end);
...
plugin:Hook("serverguard.panel.RankEditorContext", "restrictions.RankEditorContext", function(menu, uniqueID, rankData, categoryPanel, categoryBase)
	local option = menu:AddOption("Change restrictions", function()
		...
		function complete:DoClick()
			...
			local temp = table.Copy(restrictionData.Tools)
			restrictionData.Tools = {}
			for k, v in pairs(temp) do
				restrictionData.Tools[v.Command] = v.Value
			end
