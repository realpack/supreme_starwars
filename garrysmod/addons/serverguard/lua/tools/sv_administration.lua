...
local function CheckPlayerBanned(communityID, ip, serverPassword, enteredPassword, name)
	...
	if (data) then
		...
		local initialTime, initialText = util.ParseDuration((endTime - tonumber(data.startTime)) / 60);
		local minutes, text = util.ParseDuration((endTime - os.time()) / 60);
		...
		if (endTime > 0 and endTime <= os.time()) then
			...
			if (endTime <= 0) then
				return false, string.format("You are permanently banned.\n\nAdmin: %s\nReason: %s", data.admin, data.reason);
			end;
			
			return false, string.format("You have been banned for %s.\n\nAdmin: %s\nReason: %s\n\nYou will be unbanned in %s.", initialText, data.admin, data.reason, text);
		end;
	else
...
local function BAN_INSERT(steamID, communityID, playerName, reason, startTime, endTime, adminName, ipAddress)
	if (!steamID) then
		return;
	end;
...
function serverguard:BanPlayer(admin, player, length, reason, bKick, bSilent, adminNameOverride)
	...
	local adminName = adminNameOverride or bIsConsole and "Console" or serverguard.player:GetName(admin);
	...
	if (type(player) == "Player") then
		...
		hook.Run( "serverguard.PostPlayerBanned", admin, player, reason, length );
		return;
...
function serverguard:UnbanPlayer(steamID, admin)
	if (hook.Call("serverguard.PlayerUnbanned", nil, steamID, admin)) then
		return;
	end;
	...
	hook.Run( "serverguard.PostPlayerUnBanned", admin, steamID);
end;
...
-- If you encounter import errors because of this part: https://forums.gmodserverguard.com/threads/bad-argument-1-to-steamidto64.802/post-3593
