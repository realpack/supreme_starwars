...
if (SERVER) then
	...
	hook.Add("serverguard.mysql.CreateTables", "serverguard.ranks.CreateTables", function()
		local RANKS_TABLE_QUERY = serverguard.mysql:Create("serverguard_ranks");
			...
			RANKS_TABLE_QUERY:Create("color", "VARCHAR(41) NOT NULL");
    ...
    hook.Add("serverguard.mysql.UpgradeSchemas", "serverguard.ranks.UpgradeSchemas", function(callback)
	        ...
	        serverguard.mysql:UpgradeSchema("serverguard_ranks", {
				...
				{ -- update 3
	              	"ALTER TABLE `serverguard_ranks` MODIFY `color` VARCHAR(41) NOT NULL;",
	...
	-- @function[opt] fcallback The function to call when the 
	function serverguard.ranks:Save(uniqueID, name, color, immunity, texture, data, bNew, targetable, banlimit, fcallback) --
		
		local querryObj
		if (bNew) then -- we're creating a new rank
			querryObj = serverguard.mysql:Insert("serverguard_ranks");
			querryObj:Insert("unique_id", string.lower(uniqueID));
			querryObj:Insert("name", tostring(name));
			querryObj:Insert("color", util.TableToJSON(color));
			querryObj:Insert("immunity", tonumber(immunity));
			querryObj:Insert("targetable", tonumber(targetable) or 0)
			querryObj:Insert("banlimit", tonumber(banlimit) or 0)
			querryObj:Insert("texture", tostring(texture));
			querryObj:Insert("data", util.TableToJSON(data));

		else -- we're updating a already existing rank
			
			querryObj = serverguard.mysql:Update("serverguard_ranks");
			querryObj:Update("name", tostring(name));
			querryObj:Update("color", util.TableToJSON(color));
			querryObj:Update("immunity", tonumber(immunity));
			querryObj:Update("targetable", tonumber(targetable) or 0)
			querryObj:Update("banlimit", tonumber(banlimit) or 0)
			querryObj:Update("texture", tostring(texture));
			querryObj:Update("data", util.TableToJSON(data));
			querryObj:Where("unique_id", string.lower(uniqueID));
		end;

		if isfunction(fcallback) then
			querryObj:Callback(fcallback)
		end
	...
	-- @bool[opt] bNew Whether to save as a new rank, or to update an existing one with the same name. Defaults to false.
	-- @func[opt] fCallback function to call after the querry executes
	function serverguard.ranks:SaveTable(uniqueID, rankTable, bNew, fCallback) -- 
		if (type(uniqueID) == "string" and type(rankTable) == "nil") then
			rankTable = self:FindByID(uniqueID);
		end;

		self:Save(uniqueID, rankTable.name, rankTable.color, rankTable.immunity, rankTable.texture, rankTable.data, bNew, rankTable.targetable, rankTable.banlimit, fCallback);
	end;
	...
	function serverguard.ranks:SendInitialRanks(player)
		serverguard.netstream.Start(player, "sgNetworkInitialRanks", self.stored);
	end
	...
	serverguard.netstream.Hook("sgNewRank", function(player, data)
		if (serverguard.player:HasPermission(player, "Edit Ranks")) then
			...
			serverguard.ranks:SaveTable(uniqueID, nil, true, _onFinishQRR);
		end;
	end);
		...
		if (serverguard.player:HasPermission(pPlayer, "Edit Ranks")) then
            ...
            if (field == "immunity") then
                for k,v in ipairs(player.GetAll()) do
                    if (v:GetNWString "serverguard_rank" == uniqueID) then
	...
	concommand.Add("serverguard_setowner", function(player, command, arguments)
		if (util.IsConsole(player) or not game.IsDedicated()) then
	...
	serverguard.netstream.Hook("sgChangePlayerRank", function(player, data)
		if (serverguard.player:HasPermission(player, "Set Rank")) then
	...
	serverguard.netstream.Hook("sgNetworkInitialRanks", function(data)
...
function serverguard.ranks:GetData(rank, key, default)
	...
	if (rankTable) then
		if (not rankTable.data[key] and default ~= nil) then
