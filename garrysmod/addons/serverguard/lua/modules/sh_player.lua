...
function serverguard.player:SetRank(player, rank, length, bSaveless)
	...
	-- Use CAMI to support everything!
	if player:GetUserGroup() != rank then
		CAMI.SignalUserGroupChanged( player, player:GetUserGroup() or "user", rank, "serverguard" )
	end

	player.sg_incognito = nil;
	player:SetNWString("serverguard_rank", rank);
...
function serverguard.player:GetRank(player)
	...
	if (IsValid(player) and player:IsPlayer()) then
		result = player:GetNetworkedString("serverguard_rank", "user");
...
function serverguard.player:SetImmunity(player, immunity)
	player:SetNetworkedInt("serverguard_immunity", immunity);
end;
...
function serverguard.player:SetBanLimit(player, limit)
	player:SetNetworkedInt("serverguard_banlimit", limit);
end;
...
function serverguard.player:SetTargetableRank(player, immunity)
	player:SetNetworkedInt("serverguard_targetable_rank", immunity);
end;
...
function serverguard.player:GetImmunity(player)
	if (IsValid(player)) then
		return player:GetNetworkedInt("serverguard_immunity", 0);
	else
		return 100;
	end;
end;
...
function serverguard.player:GetBanLimit(player)
	if (IsValid(player)) then
		return player:GetNetworkedInt("serverguard_banlimit", 0);
	end
end;
...
function serverguard.player:GetTargetableRank(player)
	if (IsValid(player)) then
		return player:GetNetworkedInt("serverguard_targetable_rank", 0);
	else
		return 100;
	end;
end;
...
function serverguard.player:HasPermission(player, identifier)
	if (self:GetRank(player, true) == "founder") then
		return true;
	end;

	local permissionTable = serverguard.ranks:GetData(
		self:GetRank(player, true), "Permissions"
	);
