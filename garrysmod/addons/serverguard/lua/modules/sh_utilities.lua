...
function util.PlayerContainsIdentifier(player, identifier)
	...
	return (IsValid(player) and identifier and result or (string.find(string.lower(player:Name()), string.lower(identifier), 0, true) or
		string.find(string.lower(player:Nick()), string.lower(identifier), 0, true) or
		player:SteamID() == identifier or
		player:UniqueID() == identifier or
		player:SteamID64() == identifier or
		(player:IPAddress():gsub(":%d+", "")) == identifier));
