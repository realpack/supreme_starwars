...
if (SERVER) then
	...
	hook.Add("PlayerSay", "serverguard.command.PlayerSay", function(pPlayer, text, m_bToAll, m_bDead)
		...
		if (prefix == "!" or prefix == "~") then
			...
			if (pPlayer.sgMuted and commandName != "unmute") then
				return "";
			end;

			local commandExists = serverguard.command.Run(pPlayer, commandName, bSilent, unpack(arguments));
			
			if (commandExists) then
				return "";
			end;
		elseif (pPlayer.sgMuted) then
			return "";
		end;
	end);
