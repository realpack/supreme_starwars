...
-- Reverts to sqlite if module == tmysql4
if( Module == "tmysql4" ) then
	Module = "sqlite"

	timer.Create( "SG_DeprecatedSQL_Timer", 5, 0, function()
		print( "---------------------------------------------\n-----------------SERVERGUARD-----------------\n--------------------ERROR--------------------\ntmysql4 is no longer supported, please use mysqloo\n----sqlite will be used in the mean time----\n---------------------------------------------\n---------------------------------------------" )
	end )
end
...
function serverguard.mysql:RawQuery(query, callback, flags, ...)
	if (not self.connection and Module ~= "sqlite") then
		self:Queue(query);
	end;
...
--- Returns currently used SQL module
function serverguard.mysql:GetModule()
	return Module;
end;
...
hook.Add("serverguard.Initialize", "serverguard.mysql.Initialize", function()
	if( Module == "mysqloo" ) or (file.Exists("addons/serverguard_mysql.cfg", "MOD")) then
		
		local config = util.KeyValuesToTable(
			file.Read("addons/serverguard_mysql.cfg", "MOD")
		);
