...
function category:Create(base)
	...
	function button:DoClick()
		...
		local images = file.Find("materials/icon16/*.png", "MOD")
		local width = 0
		...
		for k, v in pairs(images) do
			...
			icon:SetImage("materials/icon16/" .. v)
			icon:SetSize(16, 16)
			
			function icon:DoClick()
				image = "materials/icon16/" .. v
		...
		hook.Call("serverguard.panel.RankEditorCreationMenu", nil, list);
