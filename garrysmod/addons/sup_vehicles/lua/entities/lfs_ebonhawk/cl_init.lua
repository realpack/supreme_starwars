...
function ENT:EngineActiveChanged( bActive )
	if bActive then
		self.ENG = CreateSound( self, "EHAWK_ENGINE" )
		self.ENG:PlayEx(0,0)
		
		self.DIST = CreateSound( self, "EHAWK_DIST" )
...
function ENT:Draw()
	...
	local Size = 280 + (self:GetRPM() / self:GetLimitRPM()) * 100 + Boost
		
	render.SetMaterial( mat )
	render.DrawSprite( self:LocalToWorld( Vector(-550,105,123) ), Size, Size, Color( 255, 49, 0, 255) )
	render.DrawSprite( self:LocalToWorld( Vector(-550,-105,123) ), Size, Size, Color( 255, 49, 0, 255) )
end
