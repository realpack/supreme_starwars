	...
	if self.nextEFX < CurTime() then
		...
		if emitter then
			...
			for i = 0,1 do
				...
				local vOffset = self:LocalToWorld( Vector(-310,100 * Sub,40) )
...
function ENT:Draw()
	...
	for i = 0,1 do
		local Sub = Mirror and 1 or -1
		local pos = self:LocalToWorld( Vector(-300,98 * Sub,40) )
		
		render.SetMaterial( mat )
		render.DrawSprite( pos, Size, Size, Color( 255, 42, 0, 255) )
		Mirror = true
	end
end
