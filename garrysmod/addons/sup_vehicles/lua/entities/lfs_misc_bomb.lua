...
if SERVER then
	...
	function ENT:BlindFire()
		...
		if IsValid( pObj ) then
			pObj:SetVelocityInstantaneous( self:GetForward() * (self:GetStartVelocity() + 1000) )
		end
	end
	
	function ENT:FollowTarget( followent )
		local speed = self:GetStartVelocity() + (self:GetDirtyMissile() and 3000 or 2500)
		local turnrate = (self:GetCleanMissile() or self:GetDirtyMissile()) and 70 or 60
		...
		local pos = TargetPos + followent:GetVelocity() * 0.1
	...
	function ENT:Think()	
		...
		if self.Explode then
			...
			util.BlastDamage( IsValid( Inflictor ) and Inflictor or Entity(0), IsValid( Attacker ) and Attacker or Entity(0), self:GetPos(),350,2600)
