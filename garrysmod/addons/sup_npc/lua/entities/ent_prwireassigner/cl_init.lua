...
function ENT:Initialize()
	self:SetColor(Color( 255, 80, 80, 255 ))
...
function ENT:Draw()
	...
	if IsValid(plweap) and plweap:GetClass()=="gmod_tool" and plweap:GetMode() == "patrolroutes" then
		if LocalPlayer():GetEyeTrace().Entity == self then
			...
			render.DrawSphere( pos, range, 30, 20, Color( 255, 0, 0, 60 ))
			render.CullMode(MATERIAL_CULLMODE_CCW)
			render.DrawSphere( pos, range, 20, 20, Color( 255, 0, 0, 60 ))
		end
		cam.Start3D2D(pos2,Angle(0,ang.y,90),0.5)
			draw.DrawText("Wire Assigner","default",0,0,colText,TEXT_ALIGN_CENTER)
