...
if(CLIENT) then
	...
	local function MainMenu(pnl)
		...
		ComboBox:AddChoice("Default","_default_weapon")
		ComboBox:AddChoice("Unarmed","")
		
		for k,data in pairs(list.Get("NPCUsableWeapons")) do
			ComboBox:AddChoice(data.title, data.class)
		end
		...
		local function reloadlist(List, prefix)
			if file.Exists("patrolroutes","DATA") and file.IsDir("patrolroutes","DATA") then
				...
				List:Clear()
				List:AddChoice("[No Path]");
				for k,v in pairs(files) do
					local data = string.Explode("(-)",v)
					if data[1] == prefix and data[2] == game.GetMap() then List:AddChoice(string.Explode(".txt",data[3])[1]) end
				end
		...
		RouteList = vgui.Create("DComboBox")
		RouteList:SetSortItems(false)
		reloadlist(RouteList,"route")
		pnl:AddItem(RouteList)
		local Button = pnl:AddControl("Button", {
			Label = "Reload List",
			Command = ""
		})
		if LevelSystemConfiguration then
			pnl:AddControl("Slider",{Label = "Give XP",min = 0,max = 16777215,Type = "Integer",Command = "npctool_spawner_xp"})
		end
	...
	language.Add("tool.npctool_spawner.left","Create the spawner")
	language.Add("tool.npctool_spawner.right","Create or remove a patrol point")
	...
	net.Receive("cl_npctool_spawner_spawn",function(len)
		...
		net.Start("sv_npctool_spawner_spawn")
			...
			net.WriteUInt(GetConVarNumber("npctool_spawner_xp"),24)
	...
	net.Receive("sv_npctool_spawner_spawn",function(len,pl)
		...
		local patrolRoute = net.ReadTable()
