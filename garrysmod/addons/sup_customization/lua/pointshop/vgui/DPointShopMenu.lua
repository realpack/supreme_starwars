...
local BGColor1 = Color(52, 73, 94, 150)
...
function PANEL:Init()
    ...
    closeButton:SetText('')
	closeButton:SetPos(self:GetWide() - closeButton:GetWide(), 0)
    closeButton.Paint = function( self, w, h )
        draw.RoundedBox(0, 0, 0, w, h, Color(191, 67, 57))
        draw.SimpleText('r', "marlett", w/2, h/2, Color( 255, 255, 255, 255 ), 1, 1)
    end
	...
	local function createBtn(text, material, panel, align, description)
        ...
        -- btn:SetTextStyleColor(color_white)
		...
		btn.Paint = function(pnl, w, h)
			...
			if pnl:GetActive() then
				surface.SetDrawColor(Color(245,245,245,4))
				surface.DrawRect(0, 0, w, h)
			end
		end
		btn.UpdateColours = function(pnl)
			if pnl:GetActive() then return pnl:SetTextColor(Color(180, 180, 180, 255)) end
			if pnl.Hovered then return pnl:SetTextColor(Color(200, 200, 200, 255)) end
			pnl:SetTextColor(Color(245, 245, 245, 255))
		end
	...
	for _, CATEGORY in pairs(categories) do
		...
		if IsValid( ShopCategoryTab ) then
			createBtn(CATEGORY.Name, CATEGORY.Icon, ShopCategoryTab, nil, CATEGORY.Description)
			continue
		else
        ...
        ShopCategoryTabLayout.Paint = function( self, w, h ) end

		DScrollPanel:AddItem(ShopCategoryTabLayout)

        DScrollPanel.VBar = DScrollPanel:GetVBar()
        DScrollPanel.VBar:SetWide(6)

        function DScrollPanel.VBar:PerformLayout()
        ...
        DScrollPanel.VBar.Paint = function( self, w, h ) end
        DScrollPanel.VBar.btnGrip.Paint = function( self, w, h ) draw.RoundedBox(0, 0, 0, w, h, Color(255, 255, 255, 190)) end
		...
		for _, ITEM in pairs(items) do
			if ITEM.Category == CATEGORY.Name then
                ...
                model.Paint = function( self, w, h ) end
...
local mat_currency = Material( "sup_ui/metaui/currency.png", "smooth noclamp" )
...
function PANEL:Paint(w, h)
	...
	surface.SetDrawColor( 52, 73, 94, 150 * Fraction )
    ...
    local points = string.Comma(LocalPlayer():PS_GetPoints())

    surface.SetFont('font_base_18')
    local wt, _ = surface.GetTextSize(points)
    draw.Icon( wt+20, 2, 16, 22, mat_currency, Color(255,200,10,255) )
	draw.SimpleText(points, 'font_base_18', 20, 4, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
