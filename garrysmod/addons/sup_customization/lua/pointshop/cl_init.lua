...
PS.ShopMenu = nil
PS.ClientsideModels = PS.ClientsideModel or {}
...
hook.Add('PostPlayerDraw', 'PS_PostPlayerDraw', function(ply)
	...
	if PS.ClientsideModels[ply] then
        ...
        for item_id, model in pairs(PS.ClientsideModels[ply]) do
            ...
            if ITEM.Bone == true then
                model:SetParent(ply)
                model:AddEffects(EF_BONEMERGE)
                if ply:Alive() then
                    model:SetNoDraw(ply:GetNoDraw() and true or false)
                else
            ...
            if model:GetPos():DistToSqr(ply:GetPos()) >= 128^2 then
                model:SetNoDraw(true)
            end

            -- model:SetNoDraw(ply:GetNoDraw())
        end
    end
end)
