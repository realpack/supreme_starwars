...
Возможности:
Администраторские Права + Доступ к Республиканский Дроид, Сенатор
...
Возможности:
Администраторские Права + Наёмник
...
Возможности:
Администраторские Права + Элитный Республиканский Коммандос
...
Возможности:
Администраторские Права + Коммандос
...
local rc_description = [[
ВАЖНО!

Весь донат - это прежде всего пожертвование, это значит то, что администрация может отобрать привелегии без объяснения причины
Так же ваши привелегии и сам донат может меняться в зависимости от нужд сервера.

Клон-коммандоc или Республиканский коммандос — солдат-клон Великой Армии Республики, обученный для проведения специальных операций.

В группах по четыре человека коммандос тренировались по специальной усиленной программе для выполнения специфических задач, слишком сложных для простых солдат. Обычно этими заданиями являлись скрытное проникновение на объект, разведка, ликвидация конкретных объектов и диверсии [1]. Самыми известными из коммандос являются Отряд «Дельта» и Отряд «Омега».
]]
local arc_description = [[
ВАЖНО!

Весь донат - это прежде всего пожертвование, это значит то, что администрация может отобрать привелегии без объяснения причины
Так же ваши привелегии и сам донат может меняться в зависимости от нужд сервера.

Элитный республиканский коммандос — клон-солдат Республиканской армии, обученный для проведения особо важных специальных операций.

Каминоанский контроль качества выделил два особых класса ЭРКов: класс «Нуль» (генетические манипуляции с целью улучшения боевых качеств, приведшие в том числе к меньшей лояльности к приказам) и класса «Альфа» (генетически близкие к донору — Джанго Фетту, который их и тренировал лично). Позже некоторые солдаты обоих отрядов получили звания клонов-Коммандеров.
]]
local merc_description = [[
ВАЖНО!

Весь донат - это прежде всего пожертвование, это значит то, что администрация может отобрать привелегии без объяснения причины
Так же ваши привелегии и сам донат может меняться в зависимости от нужд сервера.

Наёмник, состоящий в армии Республики
]]
...
for group,t in pairs({
	...
	["merc"] = {
		name  = "Наёмник",
		alias = "служит республике пока платят",
		desc  = merc_description,
		-- image  = "https://i.ibb.co/ZKFnPx1/1sa.jpg",
		weight = 1,
		{299}, -- 600
	},
